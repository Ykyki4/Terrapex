...
{
	// DustLashWhip.png is a vertical strip, in the layout every whip uses:
	//   y 0  h 26  handle
	//   y 26 h 16  link, thick
	//   y 42 h 16  link, thin
	//   y 58 h 16  link, glowing
	//   y 74 h 18  tip
	public class DustLashWhip : ModProjectile
	{
		...
		{
			// the whip's own tag, on top of the summon tag DefaultToWhip already applies
			target.AddBuff(ModContent.BuffType<Cracked>(), 180);
			Main.player[Projectile.owner].MinionAttackTargetNPC = target.whoAmI;
			Projectile.damage = (int)(Projectile.damage * 0.6f);   // falloff on multihit
		}
		...
		public override bool PreDraw(ref Color lightColor)
		{
			...
			for (int i = 0; i < points.Count - 1; i++)
			{
				...
				if (i == points.Count - 2)
				{
					...
					scale = MathHelper.Lerp(0.6f, 1.5f,
